...
/**
 * @author Goodwin Lu
 * @version 11/1/2018 Front-end SpaceWindow class, with buttons (Accept, Reject)
 *          and shapes. Uses ColonyCalculator's Queue in order to display the
 *          circles (working in conjunction with the two button methods), and
 *          obtains the planet's availability and capacity in order
 *          to "color in" them.
 */
public class SpaceWindow {
    ...
    public SpaceWindow(ColonyCalculator cc) throws SpaceColonyDataException {
        ...
        planet1 = new TextShape(PLANET_STARTX, PLANET_STARTY, p1.getName()
            + ", 0/10");
        System.out.println("P1 string was: " + p1.getSkill().toString());
        if (p2 != null) {
        ...
        if (colonyCalculator.getQueue().isEmpty()) {
            ...
            if (p3 != null) {
                ...
                TextShape perma3 = new TextShape(PLANET_STARTX + SQUARE_SIZE
                    * 3, PLANET_STARTY + 15, p3.getSkill().toString());
